...
class Image {
    public:
        ...
        void write_pixel(std::ostream &out, std::shared_ptr<Color> pixel) {
            ...
            auto r_value = static_cast<int>(255 * clamp(r, 0, 1));
            auto g_value = static_cast<int>(255 * clamp(g, 0, 1));
            auto b_value = static_cast<int>(255 * clamp(b, 0, 1));

            out << r_value << ' ' << g_value << ' ' << b_value << '\n';
        }
